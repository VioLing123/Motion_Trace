...
class Block_Residual(nn.Module):
    # 如果用Residual block缩小图片大小，必需要使用1×1卷积
...
class Block_Residual_New(nn.Module):
    # 如果用Residual block缩小图片大小，必需要使用1×1卷积
    def __init__(self, in_channel, out_channel, use_1x1=0, stride=1) -> None:
        super(Block_Residual_New,self).__init__()
        self.block = nn.Sequential(
            nn.Conv2d(in_channel,out_channel, kernel_size=3, stride=stride,padding= 1),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(),
...
class Simple_Residual_1(nn.Module):
    def __init__(self, in_channel, num_classes) -> None:
        super(Simple_Residual_1, self).__init__()
        #in_channel = 1, H = 1x128x128 -> 64x126x126
        self.Conv1 = nn.Conv2d(in_channel, 64, kernel_size=3, stride=2, padding=0)
        # self.maxpool1 = nn.AvgPool2d(4) # -> 64x63x63 
        self.block1 = Block_Residual_New(64,128,1,2) # -> 128x32x32
        self.block2 = Block_Residual_New(128,256,1,2) # -> 256x16x16
        self.block3 = Block_Residual_New(256,512,1,2) # -> 512x8x8
        self.aver = nn.AvgPool2d(kernel_size=8,stride=1) # ->512x1x1
        self.FL = nn.Flatten()
        self.FC = nn.Sequential(
            nn.Linear(512,256),
